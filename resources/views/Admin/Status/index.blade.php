...
@section('content')
    ...
    <div class="container container-rapi2">
        <main role="main" class="ml-sm-auto">
            <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                <h1 class="h2 h2-less">Tipos de Estado</h1>
                <div class="btn-toolbar mb-2 mb-md-0">
                    <div class="btn-group mr-2">
                        <button type="button" class="btn-less btn btn-info" id="new" onclick="AddStatus();"><i class="fas fa-plus"></i> Agregar</button>
                    </div>
                </div>
            </div>
        </main>
    ...
    <script>
        ...
        function deleted(button){
            var clave = $(button).attr('clave');
            $('#myModal').load( '{{ url('/Admin/Status/Delete') }}/'+clave,function(response, status, xhr){
                if ( status == "success" ) {
                    $('#myModal').modal('show');
                }
            } );
