    ...
    <div class="container container-rapi2">
        <main role="main" class="ml-sm-auto">
            <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                <h1 class="h2 h2-less">Reporte de Actividades</h1>
            </div>
        </main>
        <div id="Alert"></div>
    </div>
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="col-xl-12" style="padding-top: 1%;">
                    <form method="POST" action="{{route('ActivitiesPDF')}}">
                        @csrf
                            <table class="table-responsive table-card-inline">
                                <thead class="thead">
                                <tr class="tr-card-complete" style="text-align: center">
                                    <th scope="col" class="th-card filters-header filters-headers-mod"><i class="fas fa-calendar-alt"></i> Fecha de Creación</th>
                                ...
                                <tr class="tr-card-complete" style="text-align: center">
                                    ...
                                    <th scope="col" class="th-card filters-header filters-headers-mod"><i class="fas fa-users-cog"></i> Usuarios</th>
                                    <th scope="col" class="th-card filters-header filters-headers-mod"><i class="fas fa-shield-alt"></i> Estados</th>
                                </tr>
                                </thead>
                                <tbody class="fonts" style="text-align: center">
                                <tr class="tr-card-complete">
                                    <td class="td filters-act-phase filters-mod">
                                        @if(count($fases) == 0)
                                            <a style="color: #c42623; font-size: 1.2em"><strong>No hay fases</strong></a>
                                        @else
                                            @foreach($fases as $fase)
                                                <div class="form-check filters-phase-list">
                                    ...
                                    <td class="td filters-act-users filters-mod">
                                        @if(count($usuarios) == 0)
                                            <a style="color: #c42623; font-size: 1.2em"><strong>No hay fases</strong></a>
                                        @else
                                            @foreach($usuarios as $usuario)
                                                <div class="form-check filters-user-list">
                                                    <input type="checkbox" class="form-check-input box-mod" id="exampleCheck1" name="usuarios[]" value="{{$usuario->Clave}}">
                                                    <label class="form-check-label" for="exampleCheck1">{{$usuario->Nombres}}</label>
                                                </div>
                                            @endforeach
                                        @endif
                                    </td>
                                    <td class="td filters-act-status filters-mod">
                                        @foreach($estados as $estado)
                                            <div class="form-check filters-status-list">
                                                <input type="checkbox" class="form-check-input box-mod" id="exampleCheck1" name="estados[]" value="{{$estado}}">
                                                <label class="form-check-label" for="exampleCheck1">@if($estado == 0) Pendiente @elseif($estado == 1) Aprobada @elseif($estado == 2) Desaprobada @endif</label>
                                            </div>
                                        @endforeach
                                    </td>
                                </tr>
